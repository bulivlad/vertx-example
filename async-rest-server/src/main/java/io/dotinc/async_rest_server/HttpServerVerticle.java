...
public class HttpServerVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) {
        ...
        vertx.createHttpServer().requestHandler(req -> {
            vertx.eventBus()
                .send(FILE_READER_EVENT_ADDRESS.getValue(), RESPONSES_FILE_PATH.getValue() + req.path().toLowerCase(), handler -> {
                ...
                .send(HTTP_CLIENT_EVENT_ADDRESS.getValue(), REQUESTS_FILE_PATH.getValue() + req.path().toLowerCase(), handler -> {
                    if(handler.succeeded()) {
